...
public class Srl {
            ...
            for (String current : strList) {
                ...
                if (ignoreCase) {
                    if (ListElementContainsType.PREFIX.equals(type)) {
                        result = current != null && startsWithIgnoreCase(current, element);
                    } else if (ListElementContainsType.SUFFIX.equals(type)) {
                        result = current != null && endsWithIgnoreCase(current, element);
                    } else if (ListElementContainsType.KEYWORD.equals(type)) {
                        result = current != null && containsIgnoreCase(current, element);
                    } else {
                        result = equalsIgnoreCase(current, element);
                    }
                } else {
                    if (ListElementContainsType.PREFIX.equals(type)) {
                        result = current != null && startsWith(current, element);
                    } else if (ListElementContainsType.SUFFIX.equals(type)) {
                        result = current != null && endsWith(current, element);
                    } else if (ListElementContainsType.KEYWORD.equals(type)) {
                        result = current != null && contains(current, element);
                    } else {
        ...
        for (String str : strs) {
            ...
            if (ignoreCase) {
                if (KeywordType.PREFIX.equals(type)) {
                    result = str != null && startsWithIgnoreCase(str, keyword);
                } else if (KeywordType.SUFFIX.equals(type)) {
                    result = str != null && endsWithIgnoreCase(str, keyword);
                } else {
                    result = str != null && containsIgnoreCase(str, keyword);
                }
            } else {
                if (KeywordType.PREFIX.equals(type)) {
                    result = str != null && startsWith(str, keyword);
                } else if (KeywordType.SUFFIX.equals(type)) {
                    result = str != null && endsWith(str, keyword);
                } else {
                    result = str != null && contains(str, keyword);
                }
            }
            if (all) {
                if (!result) {
    ...
    public static String removeScope(final String str, final String beginMark, final String endMark) {
        ...
        while (true) {
            ...
            if (beginIndex > endIndex) {
                ...
                sb.append(rear, 0, borderIndex);
                rear = rear.substring(borderIndex);
                continue;
            }
            sb.append(rear, 0, beginIndex);
    ...
    protected static String doDecamelize(String camelName, String delimiter) {
        ...
        sb.append(camelName.substring(pos).toUpperCase());
    ...
    public static String removeLineComment(String sql) { // with removing CR!
        ...
        for (String line : lineList) {
            ...
            int realIndex = -1;
            indexLoop:
            for (DelimiterInfo delimiter : delimiterList) {
